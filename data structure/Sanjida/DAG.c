#include<stdio.h>
#include<stdlib.h>
int vartex,edge,t=0,topology[100],t_index=1,Y;
int D[100],p[100];//for dag discovery & parent
char map[100][100];
int index=1;
struct graph{
    int adj[100][100];
    int color[100];
    int discovery[100];
    int finish[100];
};
int mapping(char vname[])
{
    int i,j;
    for(i=1;i<=vartex;i++)
    {
        if(strcmp(map[i],vname)==0)
        {
            return i;
        }
    }
    strcpy(map[index++],vname);
    return index-1;
}
void dfs(struct graph *g,int start)
{
    int i,j,u;
    for(i=1;i<=vartex;i++)
    {
        g->color[i]=0;
        g->discovery[i]=0;
    }
    //for(i=1;i<=vartex;i++)
    i=start;
    while(1)
    {
        if(g->color[i]==0){
        dfs_visite(g,i);}
        //loop controll
        if(i<=vartex)
            i++;
        if(i>vartex)
            i=1;
        if(i==start)
            break;
    }
}
void dfs_visite(struct graph *g,int u)
{
    t=t+1;
    g->discovery[u]=t;
    g->color[u]=1;
    int v;
    for(v=1;v<=vartex;v++)
    {
        if(g->adj[u][v]!=0 && g->color[v]==0)
        {
            g->color[v]=1;
            dfs_visite(g,v);
        }
    }
    g->color[u]=2;
    t=t+1;
    g->finish[u]=t;
    topology[t_index++]=u;

}
///------------------------------------------
void relax(int u ,int v,int w)
{
    if(D[v]>D[u]+w)
    {
        D[v]=D[u]+w;
        p[v]=u;
    }
}
///------------------------------------------
int main()
{
    int i,j,s,d,start,wi;
     char source[100],dest[100],strt[100];
    struct graph *g=(struct graph*)malloc(sizeof(struct graph));
    freopen("dag.txt","r",stdin);
    scanf("%d %d",&vartex,&edge);
    for(i=1;i<=vartex;i++)
        for(j=1;j<=vartex;j++)
    {
        g->adj[i][j]=0;
    }
    for(i=1;i<=edge;i++)
        {
        scanf("%s %s %d",&source,&dest,&wi);
        s=mapping(source);
        d=mapping(dest);
        g->adj[s][d]=wi;
        }
    Y=1;
    ///-----------initialize
    for(i=1;i<=vartex;i++)
    {
        D[i]=999;
        p[i]=0;
    }///------------
    scanf("%s",strt);
    start=mapping(strt);
    D[start]=0;//-----------initialize
    dfs(g,start);//topologycal sort
    t=0;
int a,b;
printf("shortest paths from %s are:\n  ",strt);
for(i=vartex;i>=1;i--)
    {
        a=topology[i];
        printf("%s ",map[a]);
        for(j=1;j<=vartex;j++)
            if(g->adj[a][j]!=0)
        {
            //g->adj[a][j]=0;
            relax(a,j,g->adj[a][j]);
        }

    }
printf("\n");
    for(i=1;i<=vartex;i++)
    {
        printf("%d ", D[i]);

    }
}


